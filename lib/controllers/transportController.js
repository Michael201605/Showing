...
var TransportController = function (gcObjectAdapter, i18n, io) {
    ...
    io.on('connection', function (socket) {
        ...
        if(eventEmitter){
            eventEmitter.addListener('lineStateChanged', function (options) {

                socket.emit('lineStateChanged', options);

            });
            eventEmitter.addListener('sectionStateChanged', function (options) {
                log.debug('sectionStateChanged');
                socket.emit('sectionStateChanged', options);

            });
            eventEmitter.addListener('jobStateChanged', function (options) {
                log.debug('jobStateChanged');
        ...
        if (options.newState === GcsState.Active) {
            if (theLine) {
                if (!theLine.previousState || theLine.previousState === GcsState.Passive) {
                    ...
                    theLine.save();
                    eventEmitter.emit('lineStateChanged', {
                        lineId: theLine.id,
                ...
                if (!theSection.previousState || theSection.previousState === GcsState.Passive) {
                    if (options.jobId) {
                        Job.findOne({where: {id: options.jobId}}).then(function (theJob) {
                            ...
                            theSection.save();
                            eventEmitter.emit('sectionStateChanged', {
                            ...
                            theJob.save();
                            eventEmitter.emit('jobStateChanged', {
                                jobId: theJob.id,
