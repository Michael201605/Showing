...
var GcObjectAdapter = function (io) {
    var me = this;
...
GcObjectAdapter.prototype.monitor_a_variable_node_value = function (nodeId, callback) {
    ...
    monitoredItem.on("changed", function (dataValue) {
        ...
        if(count>1){
            ...
            callback(nodeId, dataValue,count);
        }
    });
...
GcObjectAdapter.prototype.MonitorAllGcObjects = function(callback) {
    ...
    var segments = [];
    var me = this;
    log('D','Starting to monitor all gcObjects. ');
    fs.readFile('PLC.csv', 'utf8', function (err, data) {
        if (err) {
            ...
            //remove header
            lines.splice(0, 1);
            log('D','PLC.csv , lines length: ' + lines.length);
...
GcObjectAdapter.prototype.monitor_gcObject_callback = function (monitored_node, dataValueOfMonitor) {
    var nodeData = {
        monitored_nodeId: monitored_node,
        dataValue: dataValueOfMonitor
    };
    this.io.on('connection', function (socket) {
        socket.emit('AllGcObjectsStateChanged', nodeData);
    });

};
GcObjectAdapter.prototype.monitor_sectionState_callback = function (monitored_node, dataValueOfMonitor) {
    var nodeData = {
        monitored_nodeId: monitored_node,
        dataValue: dataValueOfMonitor
    };
    this.emit('GcsSectionStateChanged', nodeData);
    // this.io.on('connection', function (socket) {
    //     socket.emit('SectionStateChanged', nodeData);
    // });

};
GcObjectAdapter.prototype.monitor_lineState_callback = function (monitored_node, dataValueOfMonitor) {
    var nodeData = {
        monitored_nodeId: monitored_node,
        dataValue: dataValueOfMonitor
    };
    this.emit('GcsLineStateChanged', nodeData);
    // this.io.on('connection', function (socket) {
