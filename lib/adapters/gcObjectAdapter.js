...
GcObjectAdapter.prototype.getItemsValue = function (nodeIds,callback) {
    the_session.readVariableValue(nodeIds, function (err, dataValue, diagnosticsInfo) {
        ...
        if (!err) {
            var nodesValue = dataValue.value.value;
            for (var i = 0; i < nodesValue.length; i++) {
                console.log(" nodesValue ", i, "  : ", nodesValue[i]);
            }
            callback(err,nodesValue);
        }
        console.log(" -----------------------");
        callback(err,null);
    });
